# 自动化计算工具 - 最小体积编译脚本
# 需要安装 MinGW-w64 或 Visual Studio + UPX (可选)

$ErrorActionPreference = "Stop"
$OutputEncoding = [System.Text.Encoding]::UTF8

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "自动化计算工具 - 最小体积编译" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (Get-Command g++ -ErrorAction SilentlyContinue) {
    ...
    $compileCmd = {
        param($output)
        # 最小体积优化参数
        g++ -std=c++17 -Os -s -ffunction-sections -fdata-sections -Wl,--gc-sections `
            -mwindows -DNDEBUG -DUNICODE -D_UNICODE `
            -fno-exceptions -fno-rtti `
            main.cpp -o $output `
            -luser32 -lgdi32 -lcomctl32 -lcomdlg32 -lshell32 -lole32 `
            -Wl,--merge-sections -Wl,--enable-stdcall-fixup
    }
}
# 检查 clang++
elseif (Get-Command clang++ -ErrorAction SilentlyContinue) {
    ...
    $compileCmd = {
        param($output)
        clang++ -std=c++17 -Os -s -ffunction-sections -fdata-sections -Wl,--gc-sections `
            -mwindows -DNDEBUG -DUNICODE -D_UNICODE `
            -fno-exceptions -fno-rtti `
            main.cpp -o $output `
            -luser32 -lgdi32 -lcomctl32 -lcomdlg32 -lshell32 -lole32
...
elseif (Get-Command cl -ErrorAction SilentlyContinue) {
    ...
    $compileCmd = {
        ...
        cl /nologo /W0 /O1 /Os /Oy /GF /GL /Gy /EHsc /DNDEBUG `
            ...
            /SUBSYSTEM:WINDOWS `
            /LTCG /OPT:REF /OPT:ICF /MERGE:.rdata=.text /MERGE:.pdata=.rdata
    }
}
...
Write-Host ""
Write-Host "正在编译（最小体积优化）..." -ForegroundColor Yellow
Write-Host ""

$outputFile = "自动化计算工具_mini.exe"
...
try {
    ...
    if (Test-Path $outputFile) {
        ...
        if (Get-Command upx -ErrorAction SilentlyContinue) {
            Write-Host ""
            Write-Host "正在使用 UPX 压缩..." -ForegroundColor Yellow
            upx --best --ultra-brute $outputFile 2>&1 | Out-Null
        } else {
            Write-Host ""
            Write-Host "提示: 安装 UPX 可进一步压缩体积" -ForegroundColor DarkGray
            Write-Host "下载地址: https://upx.github.io" -ForegroundColor DarkGray
        }
